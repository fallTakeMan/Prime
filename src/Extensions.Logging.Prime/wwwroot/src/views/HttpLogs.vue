...
const columns = ref([
  ...
  { field: "responseHeaders", header: "ResponseHeaders" },
...
const getHttpLogs = () => {
  loading.value = true;
  LogService.getHttpLogs(pageIndex.value, pageSize.value)
    .then((resp) => {
      tbl.value = resp.data;
...
<template>
  <div class="card">
    <div class="font-semibold text-xl mb-4">Http Logs</div>
    <DataTable
      ref="dt"
    ...
    >
      ...
      <template #loading> Loading http logs. Please wait. </template>
      ...
      <Column field="statusCode" header="StatusCode">
        <template #body="{ data }">
          <Tag
            ...
            :severity="getStatusSeverity(data.statusCode)"
          />
        </template>
      </Column>
      ...
      <Column field="requestBody" header="RequestBody">
        <template #body="{ data }">
          <div
            class="flex items-center gap-2"
            @click="displayPopMsg($event, data, 'requestBody')"
            v-tooltip.top="'click to view detail'"
          >
            <span class="limited-text"> {{ data.requestBody }}</span>
          </div>
        </template>
      </Column>
      <Column field="responseBody" header="ResponseBody">
        <template #body="{ data }">
          <div
            class="flex items-center gap-2"
            @click="displayPopMsg($event, data, 'responseBody')"
            v-tooltip.top="'click to view detail'"
          >
            <span class="limited-text"> {{ data.responseBody }}</span>
          </div>
        </template>
      </Column>
      <Column
    ...
    >
      <div>
        ...
        <div class="mb-2">
          <label class="font-bold">[StatusCode]: </label>
          <Tag
            :value="selectedLog.statusCode"
            :severity="getStatusSeverity(selectedLog.statusCode)"
          ></Tag>
        </div>
        <div class="mb-2">
        ...
        <Tabs value="0">
          <TabList>
            <Tab value="0">RequestBody</Tab>
            <Tab value="1">RequestHeaders</Tab>
            <Tab value="2">ResponseBody</Tab>
            <Tab value="3">ResponseHeaders</Tab>
          </TabList>
          <TabPanels>
            <TabPanel value="0">
              <p class="m-0">
